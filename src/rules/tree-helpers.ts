...
export function getSubFileEntry(directory: DirEntry, subFileName: string): FileEntry | null {
...
export function findParentModuleFilenameInTree(tree: Tree, options: SchemaOptions): string | null {
  return findFilenameInTree(
    tree.getDir(options.path),
    file =>
...
export function findParentRoutingModuleFilenameInTree(tree: Tree, options: SchemaOptions): string | null {
  return findFilenameInTree(
    tree.getDir(options.path),
    file =>
...
export function openSourceFileFromTree(tree: Tree, filename: string): typescript.SourceFile | null {
  const readFile = tree.read(filename);
  if(!readFile) return null;
  return openSourceFile(filename, () => readFile.toString('utf-8'));
}
...
export function getTouchedFiles(tree: Tree) {
  return tree.actions.reduce((files, action) => {
    const p = `.${action.path}`
    return action.path.endsWith('.ts') ? files.concat([p] as any) : files;
  }, []);
}
